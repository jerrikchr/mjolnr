# ADR-0018: Rename `smed` → `mjolnr` (Mjolnir)
...
`smed` is Danish for "smith" — a fitting metaphor (the work is shaped, held to a standard, handed back inspectable). The product has grown past a single metaphor into a full mythic vocabulary for the runtime's own concepts:
...
- **Yggdrasil** `src/graph` — deterministic code graph (files/symbols/calls) as world-tree; trunk = main lineage, branches = plans/worktrees (`smed/ext-*`, `smed/sub-*`), leaves = diffs; growth rings = durable transcript.
...
The binary name stays 6 characters either way (`smed` → `mjolnr`; English spelling `mjolnir` is the prose name, `mjolnr` the binary).

## Decision

Rename the product from `smed` to **`mjolnr`**.

1. **Binary and package:** `smed` → `mjolnr` (`Cargo.toml` `package.name`, binary name, `desktop/src-tauri/tauri.conf.json` `productName`/`identifier`, scripts, CI).
2. **Workspace/config directory:** `.smed/` → `.mjolnr/` as canonical. Ship a **compat shim for one release**: read `.mjolnr/` first, fall back to `.smed/` when absent, and **migrate on write** (first write creates `.mjolnr/` and copies forward; never delete `.smed/` automatically). Document the shim and its removal date.
3. **Code names:** keep internal `smed::` crate/module/type names stable in this ADR. Add the mythic layer for UI/copy now (`Yggdrasil` canvas, `Thing` council, runic glyphs), then rename internals incrementally behind aliases — no big-bang crate rename.
...
- No silent dropping of `.smed/` workspaces — the fallback read is mandatory until the shim's advertised removal.
...
- **Keep `smed` forever.** Understandable, but the mythic mapping is already load-bearing for how we explain the graph/council/provenance, and `smed` does not carry it.
...
- **Two binaries (`smed` + `mjolnr`) long-term.** Perpetual confusion for no benefit; one binary, one shim window.

## Migration plan (cheap-model executable)

The cheap model's checklist — mechanical, reviewable, low risk:

1. `Cargo.toml` — `name = "mjolnr"`, `[[bin]] name = "mjolnr"` (keep `smed` as alias if trivial, otherwise shim binary prints `use mjolnr`).
2. `desktop/src-tauri/tauri.conf.json` — `productName`, `identifier`, bundle.
3. Workspace discovery — `src/context/mod.rs` / `src/runtime/*` path resolution: canonical `.mjolnr/`, fallback `.smed/` read, migrate-on-write helper (unit-tested).
...
- `.smed/` → `.mjolnr/` shim removal becomes due — file a follow-up ADR to drop it.
