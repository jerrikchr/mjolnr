...
mjolnr reads MCP servers only from `.mjolnr/mcp.yaml` in the open workspace. It
...
resolved from mjolnr's process at spawn time; they are not stored in YAML,
...
they are `plugin:<name>:<tool>` via [ADR-0016](adr/0016-plugin-protocol-and-capability-modules.md) (`mjolnr plugin create` / `mjolnr plugin list`, per-file `.mjolnr/plugins/*.yaml`). End-to-end example `examples/plugins/vercel-deployments/`. Under `ask` this creates
