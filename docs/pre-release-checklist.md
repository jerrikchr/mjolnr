...
- **Repository identity** — secure the GitHub org/name `smed` and a home domain
  (`smed.com` is taken; `smed.dev` or `smed.sh`).
...
- **Documentation truth pass** — README, positioning, current capability tables
  ...
  inventory: `smed plugin create <name> [--template node|rust|python] [--yes]`,
  `smed plugin list`, `.smed/plugins/*.yaml`), release instructions, and security
