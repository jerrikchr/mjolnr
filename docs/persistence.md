...
What mjolnr relies on from SQLite and `tokio-rusqlite`, read from official sources during Phase 4 and recorded here so a later change can be checked against the contract rather than against memory.
...
2. `tokio-rusqlite`'s internal queue is **unbounded**. It cannot be mjolnr's backpressure boundary.
...
This is the whole reason mjolnr can satisfy "no blocking SQLite work in an async task" (`AGENTS.md` §4) without `spawn_blocking` at each call site: every closure body runs on that dedicated thread, never on a Tokio worker.
...
mjolnr enables `tokio-rusqlite/bundled`, which compiles SQLite from source into the binary rather than linking the host's `libsqlite3`.

- The WAL, `busy_timeout`, and `integrity_check` behaviour mjolnr tests against is the behaviour that ships, not whatever the OS supplies.
...
| Pragma | Value | Why |
|---|---|---|
| `journal_mode` | `WAL` | "The WAL journaling mode is persistent; after being set it stays in effect across multiple database connections and after closing and reopening the database." Readers do not block the writer, which is what lets a second mjolnr process read a session that another owns. |
...
- Auto-checkpoint runs at roughly 1000 pages and when the last connection closes. mjolnr does not tune this; nothing here is throughput-bound.

SQLite's single-writer rule covers the *database*. It says nothing about two mjolnr processes driving the *same session*, which is a different problem — see §5.

### 2.4 `integrity_check`

`PRAGMA integrity_check` is O(N log N) over the database. Running it at startup is forbidden, so it is only reachable through `mjolnr diagnostics --integrity`. `quick_check` is O(N) but skips index and UNIQUE consistency, which is most of what a corruption check is for; mjolnr does not offer the weaker check as if it were the real one.
...
| `PRIMARY KEY (session_id, sequence)` | Already in §9. It is the gap/duplicate-sequence guard, enforced by the database rather than by mjolnr remembering to check. |
...
2. If it is **greater** than `SCHEMA_VERSION`, refuse with `StoreError::UnsupportedSchema` **before changing journal mode or any other persistent setting**. A newer mjolnr may have written columns this build would silently ignore, and a store that half-understands its own data is worse than one that stops.
...
SQLite serialises writers to the *file*. It has no opinion about two mjolnr processes appending to the same *session*, which would interleave two runs into one transcript — a split-brain writer with a perfectly consistent database.

mjolnr therefore leases a session:
...
| `EffectUncertain` | A policy-authorised proposal or approving `ApprovalResolved`, with no `ToolCompleted`/`ToolFailed` | **The effect may or may not have happened.** mjolnr cannot tell and will not guess. Authority is retained for explanation, but certainty is the state axis. |
