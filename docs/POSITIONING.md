# mjolnr — a harness that helps builders ship

mjolnr is a coding harness you run in your project. It reads, edits, runs commands,
...
before it happens and after it lands. `mjolnr` is Danish for "smith": a short name
...
- **Trust.** The loop is built so a refusal is structural rather than requested.
  That is construction quality, not a category — it is how mjolnr is *built*, not
  what mjolnr *is*.

## A native harness with room to connect

mjolnr owns a capable native loop and talks directly to model providers. That is
...
loop. mjolnr owns its loop, so its rules are the structure of the loop itself.
...
- **Width comes from a better authorisation, not a bigger number.** Raising a
  ...
  a reflex. mjolnr's cap is four children *without an envelope* — what one human can
...
mjolnr disposes of every case instead, by intent and by evidence.
...
The incumbent harnesses expose hook callbacks, and most of mjolnr's gate *could* ride
...
itself provides it — so mjolnr *is* the loop, and other systems attach to mjolnr: tools
...
mjolnr governs two things: whether an action is permitted, and whether a completion
...
mjolnr does the work with you — reads, edits, runs, verifies — and is built so every
