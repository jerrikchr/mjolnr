...
/// `smed auth login` afterwards. The subscription caveat is unchanged —
...
/// so a user who declined once and later ran `smed init` is not treated as
...
fn mcp_config(servers: &[(String, String)]) -> String {
    let mut out = String::from(
        "# Generated by `smed onboard`. Remote MCP servers connect over HTTP.\n\
...
pub fn run_onboarding(
    ...
    if !prompt_yes_no("Set up smed now?", true) {
        record_decline();
        println!("\nNo problem — nothing was written. Run `smed init` any time to pick this up.");
        return Ok(());
    }

    let mut selections = Selections::default();

    // Step 1/6 — connect providers (reuses `smed auth login`'s machinery).
    ...
    if selections.routes.is_empty() {
        println!(
            "\nNo provider is connected yet, so there is no working session to configure.\n\
             You can still set an identity below, then run `smed auth login <provider>`\n\
             followed by `smed init --yes` when you have a key."
        );
    }
