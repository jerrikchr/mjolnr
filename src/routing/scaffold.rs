//! A starter routing scaffold for `mjolnr init` (Pillar 2).
//!
...
/// carries no ranking because `mjolnr init` refuses to invent one, this carries
...
/// A file `mjolnr init` would create: a path relative to the project root and
...
pub fn generate(seeds: &[ProviderSeed]) -> Vec<ScaffoldFile> {
    // `mjolnr init` invents no ranking: the primary route carries only `default`,
...
fn routing_config(primary: &str) -> String {
    format!(
        "# Generated by `mjolnr init`. Maps task classes and the subagent spawn\n\
...
fn route_file(provider: &ProviderId, model: &ModelId, roles: &[String], primary: bool) -> String {
    ...
    out.push_str("# Generated by `mjolnr init`. A route is an ordered fallback chain;\n");
...
mod tests {
    ...
    fn generate_is_generate_with_roles_over_empty_role_lists() {
        // The two entry points must not drift: `mjolnr init` is exactly the
